...
- **Fixed: AI features were failing on every request.** Both models the app asked for had been shut down by their providers — Google retired `gemini-2.0-flash` on 2026-06-01 and Groq retired `llama-3.3-70b-versatile` on 2026-08-16 — so every call returned 404 and every AI surface silently served its deterministic fallback. Analyst chat and briefings now use `gemini-3.6-flash` and `openai/gpt-oss-120b`. Because retired IDs were also stored on each user's saved AI profile, the transport now substitutes a supported model for any known shut-down ID, and `032_retire_shutdown_ai_models.sql` repoints existing rows.
- **Model IDs are now configurable.** `GEMINI_MODEL`, `GROQ_MODEL`, and `XAY_MODEL` override the built-in defaults, so the next provider retirement is a config change instead of a code change.
- **New `npm run ai:doctor` self-check.** Probes every configured provider with one real completion and reports exactly what each returned (missing key, HTTP 401, HTTP 404, or working), because a failed AI call is otherwise invisible behind the deterministic fallback. Exits non-zero when nothing works.
- Analyst chat now falls back to Groq and then the XAY gateway instead of stopping at Groq, logs a sanitized attempt trace when every provider fails, and records the provider that actually answered (it previously stored the requested model name, so the logs could not distinguish a real answer from a fallback).
...
- **XAY briefing gateway.** Scheduled and on-demand briefings now prefer XAY's OpenAI-compatible gateway, then fall back to direct Groq and Gemini.
